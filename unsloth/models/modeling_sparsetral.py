# coding=utf-8
# Copyright 2023 Sparsetral AI and the HuggingFace Inc. team. All rights reserved.
#
...
# Copied from transformers.models.llama.modeling_llama.LlamaRMSNorm with Llama->Sparsetral
class SparsetralRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        SparsetralRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
# Copied from transformers.models.llama.modeling_llama.LlamaRotaryEmbedding with Llama->Sparsetral
class SparsetralRotaryEmbedding(nn.Module):
...
class SparsetralAttention(nn.Module):
    ...
    def __init__(self, config: SparsetralConfig, layer_idx: Optional[int] = None):
        ...
        self.rotary_emb = SparsetralRotaryEmbedding(
            self.head_dim,
...
class SparsetralFlashAttention2(SparsetralAttention):
    """
    Sparsetral flash attention module. This module inherits from `SparsetralAttention` as the weights of the module stays
...
# Copied from transformers.models.llama.modeling_llama.LlamaSdpaAttention with Llama->Sparsetral
class SparsetralSdpaAttention(SparsetralAttention):
    """
    Sparsetral attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `SparsetralAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from SparsetralAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "SparsetralModel is using SparsetralSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
MISTRAL_ATTENTION_CLASSES = {
    "eager": SparsetralAttention,
    "flash_attention_2": SparsetralFlashAttention2,
    "sdpa": SparsetralSdpaAttention,
}
...
class SparsetralDecoderLayer(nn.Module):
    def __init__(self, config: SparsetralConfig, layer_idx: int):
        ...
        self.mlp = SparsetralMLP(config)
        self.input_layernorm = SparsetralRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = SparsetralRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
@add_start_docstrings(
    "The bare Sparsetral Model outputting raw hidden-states without any specific head on top.",
    MISTRAL_START_DOCSTRING,
)
class SparsetralPreTrainedModel(PreTrainedModel):
    config_class = SparsetralConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["SparsetralDecoderLayer"]
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, SparsetralModel):
...
class SparsetralModel(SparsetralPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`SparsetralDecoderLayer`]
    ...
    def __init__(self, config: SparsetralConfig):
        ...
        self.layers = nn.ModuleList(
            [
                SparsetralDecoderLayer(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = SparsetralRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        ...
        if (
            ...
            if is_padding_right:
                raise ValueError(
                    ...
                    " this may lead to unexpected behaviour for Flash Attention version of Sparsetral. Make sure to "
...
class SparsetralForCausalLM(SparsetralPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.model = SparsetralModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, SparsetralForCausalLM

        >>> model = SparsetralForCausalLM.from_pretrained("mistralai/Sparsetral-7B-v0.1")
        >>> tokenizer = AutoTokenizer.from_pretrained("mistralai/Sparsetral-7B-v0.1")
