...
from .modeling_sparsetral import (
    SparsetralAttention,
    SparsetralDecoderLayer,
    SparsetralModel,
    SparsetralForCausalLM,
...
try:
    from .modeling_sparsetral import (
        SparsetralSdpaAttention,
        SparsetralFlashAttention2,
    )
except:
    SparsetralSdpaAttention = SparsetralAttention
    SparsetralFlashAttention2 = SparsetralAttention
pass


def SparsetralAttention_fast_forward(
    self,
...
def SparsetralDecoderLayer_fast_forward(
    self,
    hidden_states: torch.Tensor,
    causal_mask: Optional[xformers.attn_bias.BlockDiagonalCausalMask] = None,
    attention_mask: Optional[torch.Tensor] = None,
...
def SparsetralModel_fast_forward(
    self,
...
def SparsetralForCausalLM_fast_forward(
    self,
    input_ids: torch.LongTensor = None,
    causal_mask: Optional[xformers.attn_bias.BlockDiagonalCausalMask] = None,
    attention_mask: Optional[torch.Tensor] = None,
...
class FastSparsetralModel(FastLlamaModel):

    @staticmethod
    def pre_patch():
        SparsetralAttention.forward = SparsetralAttention_fast_forward
        SparsetralSdpaAttention.forward = SparsetralAttention_fast_forward
        SparsetralFlashAttention2.forward = SparsetralAttention_fast_forward
        SparsetralDecoderLayer.forward = SparsetralDecoderLayer_fast_forward
        SparsetralModel.forward = SparsetralModel_fast_forward
        SparsetralForCausalLM.forward = SparsetralForCausalLM_fast_forward
    ...
    def from_pretrained(
        ...
        rope_scaling=None,  # Sparsetral does not support RoPE scaling
        fix_tokenizer=True,
        **kwargs,
    ):
        # Sparsetral does NOT support RoPE Scaling!
        if rope_scaling is not None:
            logger.warning_once(
                "Unsloth: Sparsetral models do not support RoPE scaling."
            )
        pass
        ...
        statistics = (
            f"==((====))==  Unsloth: Fast Sparsetral patching release {__version__}\n"
        ...
        model_config = SparsetralConfig.from_pretrained(model_name, token=token)
        ...
        # Sparsetral does NOT support RoPE Scaling sadly so we have to error out.
        if max_seq_length > model_max_seq_length:
            raise RuntimeError(
                "Unsloth: Unfortunately Sparsetral type models do not support RoPE scaling!\n"
        ...
        model = SparsetralForCausalLM.from_pretrained(
            model_name,
            config=model_config,
        ...
        model = FastSparsetralModel.post_patch(model)
    ...
    def get_peft_model(
        ...
        use_gradient_checkpointing=True,
        random_state=741,
        max_seq_length=4096,  # not used anymore
        use_rslora=False,
        ...
        **kwargs,
        ...
        model = FastSparsetralModel.patch_peft_model(model, use_gradient_checkpointing)
        return model
