...
def unsloth_save_model(
    ...
    state_dict["model.embed_tokens.weight"] = (
        internal_model.model.embed_tokens.weight.data
    )
    ...
    for param_tensor in model.state_dict():
        ...
        #     moe_state.update({param_tensor: model.state_dict()[param_tensor]})
    if moe_state != {}:
        moe_model_path = os.path.join(save_directory, "moe_model.bin")
        torch.save(moe_state, moe_model_path)

    # Print location
    if push_to_hub:
        print(
            f"Saved to https://huggingface.co/{username}/{save_directory.lstrip('/')}"
        )
    pass
...
def install_llama_cpp_make_non_blocking():
    ...
    run_installer = subprocess.Popen(
        full_command, env=env, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
    )
    return run_installer
...
def save_to_gguf(
    ...
    pass
    print(
        f"Unsloth: [1] Converting HF into {first_conversion} GGUF format. This will take 3 minutes..."
    )
    ...
    if quantization_method != first_conversion:
        ...
        with subprocess.Popen(
            command, shell=True, stderr=subprocess.PIPE, bufsize=1
        ) as sp:
...
def upload_to_huggingface(
    model, save_directory, token, method, extra="", file_location=None
):
    # Check for username
    username = ""
...
def patch_saving_functions(model):
    ...
    if hasattr(model, "_original_push_to_hub"):
        return
    ...
    while True:
        if hasattr(original_model, "_original_push_to_hub"):
            ...
            if hasattr(original_model, "push_to_hub_merged"):
                del original_model.push_to_hub_merged
            if hasattr(original_model, "save_pretrained_merged"):
                del original_model.save_pretrained_merged
            if hasattr(original_model, "push_to_hub_gguf"):
                del original_model.push_to_hub_gguf
            if hasattr(original_model, "save_pretrained_gguf"):
                del original_model.save_pretrained_gguf
        pass
